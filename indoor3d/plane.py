...
class PlaneIndoor:
    ...
    def __repr__(self):
        return f"PlaneIndoor(A={self.A},B={self.B},C={self.C},D={self.D})"


def get_angle_between_planes(plane_1: Type[PlaneIndoor],
                             plane_2: Type[PlaneIndoor]) -> float:
    """
    ...
    :type plane_1: Type[PlaneIndoor]
    :param plane_2: Second plane.
    :type plane_2: Type[PlaneIndoor]
    ...
    ::
        ...
        >>> import plane
        >>> plane_1 = plane.PlaneIndoor((1, 2, 3, 4))
        >>> plane_2 = plane.PlaneIndoor((3, 6, -5, 10))
        >>> angle = plane.get_angle_between_planes(plane_1, plane_2)
        >>> angle
        1.5707963267948966
    """
...
def check_planes_paralell(plane_1: Type[PlaneIndoor],
                          plane_2: Type[PlaneIndoor],
    ...
    ::
        ...
        >>> plane_1 = plane.PlaneIndoor((1, 2, 3, 4))
        >>> plane_2 = plane.PlaneIndoor((3, 6, -5, 10))
        >>> plane.check_planes_paralell(plane_1, plane_2)
        False
        >>> plane_1 = plane.PlaneIndoor((1, 2, 3, 4))
        >>> plane_2 = plane.PlaneIndoor((-1, -2, -4, 10))
        >>> plane.check_planes_paralell(plane_1, plane_2) # max_tolerance_degrees is 10 by default.
        True
        >>> plane.check_planes_paralell(plane_1, plane_2, max_tolerance_degrees = 5)
        False
    '''
...
def check_planes_perpendicular(plane_1: Type[PlaneIndoor],
                               plane_2: Type[PlaneIndoor],
                               max_tolerance_degrees: float = 10) -> bool:
    """
    ...
    ::
        ...
        >>> import plane
        >>> plane_1 = plane.PlaneIndoor((1, 2, 3, 4))
        >>> plane_2 = plane.PlaneIndoor((1, 2, 3, 10))
        >>> plane.check_planes_perpendicular(plane_1, plane_2)
        False
        >>> plane_1 = plane.PlaneIndoor((1, 2, 3, 4))
        >>> plane_2 = plane.PlaneIndoor((3, 6, -4, 10))
        >>> plane.check_planes_perpendicular(plane_1, plane_2) # max_tolerance_degrees is 10 by default.
        True
        >>> plane.check_planes_perpendicular(plane_1, plane_2, max_tolerance_degrees = 5)
        False
    """
    max_angle_radians = max_tolerance_degrees * math.pi / 180
...
def get_point_on_plane_closest_to_the_origin(plane: Type[PlaneIndoor]) -> np.ndarray:
    """
    ...
    ::
        ...
        >>> point = plane.get_point_on_plane_closest_to_the_origin(plane)
        >>> point
...
def get_distance_between_plane_and_point(plane: Type[PlaneIndoor],
    ...
    ::
        ...
        >>> import plane
        >>> plane = plane.PlaneIndoor((1, 2, 3, 4))
        >>> distance = plane.get_distance_between_plane_and_point(plane, [1, 2, 3])
        >>> distance
        4.810702354423639
    """
    a, b, c, d = plane.A, plane.B, plane.C, plane.D
...
def get_distance_between_plane_and_origin(plane: Type[PlaneIndoor]) -> float:
    """
    ...
    ::
        ...
        >>> import plane
        >>> plane = plane.PlaneIndoor((1, 2, 3, 4))
        >>> distance = plane.get_distance_between_plane_and_origin(plane)
        >>> distance
        1.0690449676496976
    """
...
def get_distance_between_parallel_planes(plane_1: Type[PlaneIndoor],
                                         plane_2: Type[PlaneIndoor]) -> float:
    """
    ...
    ::
        ...
        >>> plane_1 = plane.PlaneIndoor((1, 2, 3, 4))
        >>> plane_2 = plane.PlaneIndoor((1, 2, 3, 10))
        >>> distance = plane.get_distance_between_parallel_planes(plane_1, plane_2)
        >>> distance
        1.6035674514745464
    """
...
def get_distance_and_sign_between_plane_and_point(plane: Type[PlaneIndoor],
    ...
    ::
        ...
        >>> import plane
        >>> plane = plane.PlaneIndoor((1, 2, 3, 4))
        >>> distance, sign = plane.get_distance_and_sign_between_plane_and_point(plane, [1, 2, 3])
        >>> distance
        4.810702354423639
        >>> sign
        18
...
def get_angle_between_plane_and_axis(plane: Type[PlaneIndoor], axis: str) -> float:
    '''
    ...
    ::
        ...
        >>> import plane
        >>> plane = plane.PlaneIndoor((1, 2, 3, 4))
        >>> angle = plane.get_angle_between_plane_and_axis(plane, axis = "X")
        >>> angle
        0.2705497629785729
        >>> angle = plane.get_angle_between_plane_and_axis(plane, axis = "Y")
        >>> angle
        0.5639426413606289
        >>> angle = plane.get_angle_between_plane_and_axis(plane, axis = "Z")
        >>> angle
        0.9302740141154721
        >>> plane = plane.PlaneIndoor((0.1, 0.1, 1, 4))
        >>> angle = plane.get_angle_between_plane_and_axis(plane, axis = "X")
        >>> angle
        0.09917726107940236
        >>> angle = plane.get_angle_between_plane_and_axis(plane, axis = "Y")
...
def get_angle_plane_with_plane_axis_equal_0(plane: Type[PlaneIndoor], axis: str) -> float:
    """
    ...
    ::
        ...
        >>> import plane
        >>> plane = plane.PlaneIndoor((1, 2, 3, 4))
        >>> angle = plane.get_angle_plane_with_plane_axis_equal_0(plane, axis = "X")
        >>> angle
        1.3002465638163236
        >>> angle = plane.get_angle_plane_with_plane_axis_equal_0(plane, axis = "Y")
        >>> angle
        1.0068536854342678
        >>> angle = plane.get_angle_plane_with_plane_axis_equal_0(plane, axis = "Z")
        >>> angle
        0.6405223126794245
        >>> plane = plane.PlaneIndoor((0.1, 0.1, 1, 4))
        >>> angle = plane.get_angle_plane_with_plane_axis_equal_0(plane, axis = "X")
        ...
        >>> angle
        0.1404897017535205
        >>> plane = plane.PlaneIndoor((0, 0, 1, 4))
        >>> angle = plane.get_angle_plane_with_plane_axis_equal_0(plane, axis = "X")
        >>> angle
        1.5707963267948966
        >>> angle = plane.get_angle_plane_with_plane_axis_equal_0(plane, axis = "Y")
    ...
    assert (axis_param in {"X", "Y", "Z"})
    if axis_param == "X":
        plane_axis = PlaneIndoor((1, 0, 0, 0))
    elif axis_param == "Y":
        plane_axis = PlaneIndoor((0, 1, 0, 0))
    else:
        plane_axis = PlaneIndoor((0, 0, 1, 0))
    return get_angle_between_planes(plane, plane_axis)


def get_plane_parallel_to_given_plane_through_a_point(plane: Type[PlaneIndoor], point: Iterable[float]) -> Type[
    PlaneIndoor]:
    """
    ...
    ::
        ...
        >>> import plane
        >>> plane = plane.PlaneIndoor((1, 2, 3, 4))
        >>> plane_parallel = plane.get_plane_parallel_to_given_plane_through_a_point(plane, point = [1,1,1])
        >>> plane_parallel
        PlaneIndoor(A=1,B=2,C=3,D=-6)
    """
    a, b, c, d = plane.A, plane.B, plane.C, plane.D
    x, y, z = point[0], point[1], point[2]
    new_d = -(a * x + b * y + c * z)
    return PlaneIndoor((a, b, c, new_d))


def get_point_projection_onto_plane(plane: Type[PlaneIndoor], point: Iterable[float]) -> np.ndarray:
    """
     ...
     ::
         ...
         >>> import plane
         >>> plane = plane.PlaneIndoor((1, 2, 3, 4))
         >>> projection = plane.get_point_projection_onto_plane(plane, point = [1,1,1])
         >>> projection
...
def get_point_intersection_of_three_planes(plane_1: Type[PlaneIndoor], plane_2: Type[PlaneIndoor],
                                           plane_3: Type[PlaneIndoor]) -> np.ndarray:
    """
    ...
    ::
        ...
        >>> import plane
        >>> plane_1 = plane.PlaneIndoor((1, 0, 0, -1))
        >>> plane_2 = plane.PlaneIndoor((0, 1, 0, -1))
        >>> plane_3 = plane.PlaneIndoor((0, 0, 1, -1))
        >>> point = plane.get_point_intersection_of_three_planes(plane_1, plane_2, plane_3)
        >>> point
        array([1., 1., 1.])
    """
...
def get_plane_perpendicular_to_two_planes(plane_1: Type[PlaneIndoor], plane_2: Type[PlaneIndoor]) -> Type[PlaneIndoor]:
    """
    ...
    :type plane_model_2: Type[PlaneIndoor]
    :return: Plane perpendicular to the input planes.
    :type: Type[PlaneIndoor]
    ...
    ::
        ...
        >>> import plane
        >>> plane_1 = plane.PlaneIndoor((1, 2, 3, 4))
        >>> plane_2 = plane.PlaneIndoor((3, 1, -5/3, 0))
        >>> new_plane = plane.get_plane_perpendicular_to_two_planes(plane_1, plane_2)
        >>> new_plane
        PlaneIndoor(A=-0.4735225469652655,B=0.7975116580467629,C=-0.3738335897094201,D=0)
    """
    v_perp = np.cross(plane_1.normal, plane_2.normal)
    return PlaneIndoor((v_perp[0], v_perp[1], v_perp[2], 0))


def get_distance_and_sign_between_plane_and_points(plane: Type[PlaneIndoor], points: Iterable[Iterable[float]]) -> Tuple[
    ...
    ::
        ...
        >>> distances, signs = plane.get_distance_and_sign_between_plane_and_points(plane, points)
        >>> distances
...
def get_planes_parallel_to_given_distance_of_plane(plane: Type[PlaneIndoor], distance: float) -> Tuple[
    Type[PlaneIndoor], Type[PlaneIndoor]]:
    """
    ...
    :rtype: Tuple[Type[PlaneIndoor], Type[PlaneIndoor]]
    ...
    ::
        ...
        >>> plane = plane.PlaneIndoor((1, 1, 1, 1))
        >>> distance = 1
        >>> plane_1, plane_2 = plane.get_planes_parallel_to_given_distance_of_plane(plane, distance = 1)
        >>> plane_1
        PlaneIndoor(A=1,B=1,C=1,D=-0.7320508075688776)
        >>> plane_2
        PlaneIndoor(A=1,B=1,C=1,D=2.7320508075688776)
    """
...
def check_plane_parallel_to_all_planes_in_list(plane: Type[PlaneIndoor],
                                               list_planes: List[Type[PlaneIndoor]],
                                               max_tolerance_degrees: float = 10) -> bool:
    """
    ...
    :type list_planes: List[Type[PlaneIndoor]]
    :param max_tolerance_degrees: Tolerance for the angle between normals.
    :type max_tolerance_degrees: float
    ...
    ::
        ...
        >>> import plane
        >>> plane = plane.PlaneIndoor((1, 2, 3, 4))
        >>> plane_1 = plane.PlaneIndoor((2, 4, 6, 12))
        >>> plane_2 = plane.PlaneIndoor((-11, -21, -31, 75))
        >>> plane_3 = plane.PlaneIndoor((3.2, 6.2, 9.2, 1))
        >>> list_planes = [plane_1, plane_2, plane_3]
        >>> plane.check_plane_parallel_to_all_planes_in_list(plane, list_planes)
        True
        >>> plane.check_plane_parallel_to_all_planes_in_list(plane, list_planes, max_tolerance_degrees=0)
        False
    """
...
def get_partition_of_list_of_planes_by_parallelism(list_planes: List[Type[PlaneIndoor]],
                                                   max_tolerance_degrees: float = 10) -> \
        List[List[Type[PlaneIndoor]]]:
    """
    ...
    :rtype: List[List[Type[PlaneIndoor]]]
    ...
    ::
        ...
        >>> import plane
        >>> plane_1 = plane.PlaneIndoor((2, 4, 6, 12))
        >>> plane_2 = plane.PlaneIndoor((1, 11, 21, 5))
        >>> plane_3 = plane.PlaneIndoor((-11, -21, -31, 75))
        >>> plane_4 = plane.PlaneIndoor((-10.3, -110.4, -210.5, 34))
        >>> plane_5 = plane.PlaneIndoor((3.2, 6.2, 9.2, 1))
        >>> plane_6 = plane.PlaneIndoor((12, 37, 14, 12))
        >>> list_planes = [plane_1, plane_2, plane_3, plane_4, plane_5, plane_6]
        >>> partition = plane.get_partition_of_list_of_planes_by_parallelism(list_planes)
        >>> for part in partition: print (part)
        [PlaneIndoor(A=2,B=4,C=6,D=12), PlaneIndoor(A=-11,B=-21,C=-31,D=75), PlaneIndoor(A=3.2,B=6.2,C=9.2,D=1)]
        [PlaneIndoor(A=1,B=11,C=21,D=5), PlaneIndoor(A=-10.3,B=-110.4,C=-210.5,D=34)]
        [PlaneIndoor(A=12,B=37,C=14,D=12)]
        >>> partition = plane.get_partition_of_list_of_planes_by_parallelism(list_planes, max_tolerance_degrees=0.5)
        >>> for part in partition: print (part)
        [PlaneIndoor(A=2,B=4,C=6,D=12)]
        [PlaneIndoor(A=1,B=11,C=21,D=5), PlaneIndoor(A=-10.3,B=-110.4,C=-210.5,D=34)]
        [PlaneIndoor(A=-11,B=-21,C=-31,D=75), PlaneIndoor(A=3.2,B=6.2,C=9.2,D=1)]
        [PlaneIndoor(A=12,B=37,C=14,D=12)]
        >>> partition = plane.get_partition_of_list_of_planes_by_parallelism(list_planes, max_tolerance_degrees=0)
        >>> for part in partition: print (part)
        [PlaneIndoor(A=2,B=4,C=6,D=12)]
        [PlaneIndoor(A=1,B=11,C=21,D=5)]
        [PlaneIndoor(A=-11,B=-21,C=-31,D=75)]
        [PlaneIndoor(A=-10.3,B=-110.4,C=-210.5,D=34)]
        [PlaneIndoor(A=3.2,B=6.2,C=9.2,D=1)]
        [PlaneIndoor(A=12,B=37,C=14,D=12)]
    """
...
def check_plane_perpendicularity_between_lists_of_planes(list_planes_1: List[Type[PlaneIndoor]],
                                                         list_planes_2: List[Type[PlaneIndoor]],
                                                         max_tolerance_degrees: float = 10,
    ...
    :type list_planes_1: List[Type[PlaneIndoor]]
    :param list_planes_2: Second list of planes.
    :type list_planes_2: List[Type[PlaneIndoor]]
    ...
    ::
        ...
        >>> import plane
        >>> list_planes_1 = [plane.PlaneIndoor((1, 2, 3, 4)), plane.PlaneIndoor((3, 6.1, 9.2, 12))]
        >>> list_planes_2 = [plane.PlaneIndoor((3, 6, -4, 10)), plane.PlaneIndoor((6.1, 12.3, -8.3, -35))]
        >>> plane.check_plane_perpendicularity_between_lists_of_planes(list_planes_1, list_planes_2, return_all_values = True)
        (True, [True, True, True, True])
        >>> plane.check_plane_perpendicularity_between_lists_of_planes(list_planes_1, list_planes_2, max_tolerance_degrees= 2, return_all_values = True)
        (False, [False, False, False, False])
        >>> plane.check_plane_perpendicularity_between_lists_of_planes(list_planes_1, list_planes_2, max_tolerance_degrees= 5.5, return_all_values = True)
        (True, [False, False, False, True])
        >>> plane.check_plane_perpendicularity_between_lists_of_planes(list_planes_1, list_planes_2, max_tolerance_degrees= 5.5, min_percentage = 0.5, return_all_values = True)
        (False, [False, False, False, True])
        >>> plane.check_plane_perpendicularity_between_lists_of_planes(list_planes_1, list_planes_2, max_tolerance_degrees= 5.5, min_percentage = 0.5)
        False
    """
...
def get_lists_of_planes_perpendicular_to_each_other_in_partition_first_two(partition: List[List[Type[PlaneIndoor]]],
                                                                           max_tolerance_degrees: float = 10,
                                                                           min_percentage: float = 0) \
        -> Optional[Tuple[List[Type[PlaneIndoor]], List[Type[PlaneIndoor]]]]:
    """
    ...
    :type partition: List[List[Type[PlaneIndoor]]]
    :param max_tolerance_degrees: Tolerance for the angle between normals, default is 10 degrees.
    :type max_tolerance_degrees: float
    ...
    :rtype: Optional[Tuple[List[Type[PlaneIndoor]], List[Type[PlaneIndoor]]]]
    ...
    ::
        ...
        >>> import plane
        >>> list_planes_1 = [plane.PlaneIndoor((1, 2, 3, 4)), plane.PlaneIndoor((3, 6.1, 9.2, 12))]
        >>> list_planes_2 = [plane.PlaneIndoor((7, 20, 3, 4)), plane.PlaneIndoor((21, 61.6, 9.2, 12))]
        >>> list_planes_3 = [plane.PlaneIndoor((3, 6, -4, 10)), plane.PlaneIndoor((6.1, 12.3, -8.3, -35))]
        >>> partition = [list_planes_1, list_planes_2, list_planes_3]
        >>> list_planes = plane.get_lists_of_planes_perpendicular_to_each_other_in_partition_first_two(partition)
        >>> for list_plane in list_planes: print(list_plane)
        [PlaneIndoor(A=1,B=2,C=3,D=4), PlaneIndoor(A=3,B=6.1,C=9.2,D=12)]
        [PlaneIndoor(A=7,B=20,C=3,D=4), PlaneIndoor(A=21,B=61.6,C=9.2,D=12)]
    """
